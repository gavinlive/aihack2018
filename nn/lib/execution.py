...
class execution(object):
  ...
  def evaluate(self, max_steps=None, max_epochs=None, save_step=None, session=None, checkpoint_path=None):
    ...
    def run_evaluation(last_checkpoint_path):
        ...
        input_datas = []
        all_diagnostics = []
        for i in range(self.data_strap.n_splits_per_gpu_test[0]): # [] This is cheating and needs to be fixed ):    self.data_strap.n_splits_per_gpu_test[0]
            feed_dict = {}
            for gpu in range(self.data_strap.num_gpus):
              ...
              #print(">>>>>Extra data feed_dict")
              for key in self.data_strap.extra_data._fields:
                  feed_dict["ExtraData_"+key+"GPU"+str(gpu)+":0"] = self.data_strap.fetch_data('test',key,gpu,i)
              for key in self.data_strap.extra_data._fields:
                  feed_dict["ValidationExtraData_"+key+"GPU"+str(gpu)+":0"] = self.data_strap.fetch_data('test',key,gpu,0) # needs fixing to a proper validation set
            print("data split: %d of %d" % (i+1, self.data_strap.n_splits_per_gpu_test[0]))
        ...
        zipped_diag = list(zip(diag['mse'], diag['accuracy'], diag['moe_acc']))
        with open(self.summary_folder + '/diagnostics.csv', 'w') as the_file:
            the_file.write('mse, e_acc, moe_acc\n')
            for entry in zipped_diag:
                for val in list(entry):
                    the_file.write(str(val))
                    the_file.write(',')
                the_file.write('\n')
        with open(self.summary_folder + '/diagnostics2.csv', 'w') as the_file:
            the_file.write('x, y, gt, e_acc, moe_acc, gt_error\n')
            for i in range(len(results)):
                the_file.write(str(i))
                the_file.write(',')
                the_file.write(str(main_results["y"][i][0]))
                the_file.write(',')
                the_file.write(str(main_results["gt"][i][0]))
                the_file.write(',')
                the_file.write(str(np.divide(np.abs(main_results["y"][i][0] - main_results["gt"][i][0]), main_results["gt"][i][0]) * 100. ))
                the_file.write(',')
                the_file.write(str(np.divide(np.abs(main_results["y"][i][1] - main_results["gt"][i][1]), main_results["gt"][i][1]) * 100. ))
                the_file.write(',')
                the_file.write(str(main_results["gt"][i][1]))
                the_file.write(',')
                the_file.write('\n')
